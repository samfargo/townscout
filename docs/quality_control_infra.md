...
### Drive-time Confidence Boosters
- Maintain a tiny golden dataset (≤25 hexes × categories) with hand-verified drive times pulled from external routing APIs; compare against `T_hex + D_anchor` totals after every `make minutes` run and trigger a failure if deltas exceed 10%.
- When adding new POI categories or large geography expansions, rerun the golden check plus one real-world “decision journey” scenario (e.g., suburb-to-airport) before release.
