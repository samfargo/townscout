...
def process(
    *,
    ...
    """Main processing entrypoint. Returns the output parquet path."""
    prism_dir = prism_dir.expanduser()
    output = output.expanduser()
    output.parent.mkdir(parents=True, exist_ok=True)

    variables = ("tmean", "tmin", "tmax", "ppt")
    rasters = discover_rasters(prism_dir, variables)

    frames: list[pl.DataFrame] = []
    for res in resolutions:
        gdf = build_hex_frame(minutes_glob, res)
        geometry_cache: dict[str | None, gpd.GeoDataFrame] = {}
        data: dict[str, Iterable[float] | Iterable[str] | Iterable[int]] = {
