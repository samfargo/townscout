...
if [[ "${TARGET}" == "all" ]]; then
  ...
  [[ -d "${LOCAL_RESULTS_DIR}/data/anchors" ]] && rsync -a "${LOCAL_RESULTS_DIR}/data/anchors/" "${REPO_ROOT}/data/anchors/" || log "warning: data/anchors not found in remote results"
  [[ -d "${LOCAL_RESULTS_DIR}/data/minutes" ]] && rsync -a "${LOCAL_RESULTS_DIR}/data/minutes/" "${REPO_ROOT}/data/minutes/" || log "warning: data/minutes not found in remote results"
  [[ -d "${LOCAL_RESULTS_DIR}/tiles" ]] && rsync -a "${LOCAL_RESULTS_DIR}/tiles/" "${REPO_ROOT}/tiles/" || log "warning: tiles not found in remote results"
  [[ -d "${LOCAL_RESULTS_DIR}/data/d_anchor_category" ]] && rsync -a "${LOCAL_RESULTS_DIR}/data/d_anchor_category/" "${REPO_ROOT}/data/d_anchor_category/" || log "warning: data/d_anchor_category not found in remote results"
  [[ -d "${LOCAL_RESULTS_DIR}/data/d_anchor_brand" ]] && rsync -a "${LOCAL_RESULTS_DIR}/data/d_anchor_brand/" "${REPO_ROOT}/data/d_anchor_brand/" || log "warning: data/d_anchor_brand not found in remote results"
fi
