...
const Slider = React.forwardRef<React.ElementRef<typeof SliderPrimitive.Root>, SliderProps>(
  ({ className, withValue = false, ...props }, ref) => {
    const handleCount = React.useMemo(() => {
      const valueLength = props.value?.length ?? props.defaultValue?.length;
      return valueLength && valueLength > 0 ? valueLength : 1;
        ...
        >
          ...
          {Array.from({ length: handleCount }).map((_, index) => (
            <SliderPrimitive.Thumb
              ...
              key={index}
              className="block h-4 w-4 rounded-full border border-amber-900 bg-[#f8f1e2] shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-700"
            />
          ))}
        </SliderPrimitive.Root>
        {withValue && (
