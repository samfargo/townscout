...
- Hex summaries include climate quantiles and a `near_power_corridor` boolean generated by `src/03f_compute_power_corridors.py`, powering the “Avoid power lines” toggle in the frontend.
...
6. **High-Voltage Corridor Overlay**

* Run `src/03f_compute_power_corridors.py` to buffer high-voltage OSM `power=line` features, dissolve the corridor, and intersect with the H3 grid.
...
```bash
make pois anchors minutes power_corridors climate d_anchor_category d_anchor_brand merge tiles
```
